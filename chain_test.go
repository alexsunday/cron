...
func appendingJob(slice *[]int, value int) Job {
	var m sync.Mutex
	return FuncJob(func(_ time.Time) {
		m.Lock()
...
func appendingWrapper(slice *[]int, value int) JobWrapper {
	return func(j Job) Job {
		return FuncJob(func(cur time.Time) {
			appendingJob(slice, value).Run(cur)
			j.Run(cur)
		})
	}
}
...
func TestChainRecover(t *testing.T) {
	panickingJob := FuncJob(func(_ time.Time) {
...
func (j *countJob) Run(cur time.Time) {
	j.m.Lock()
	j.started++
	j.m.Unlock()
	time.Sleep(j.delay)
