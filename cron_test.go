...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", func(t time.Time) { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(cur time.Time) { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("* * * * * ?", func(cur time.Time) { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("@every 2s", func(cur time.Time) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", func(cur time.Time) {})
	cron.AddFunc("* * * * * ?", func(cur time.Time) { wg.Done() })
	id1, _ := cron.AddFunc("* * * * * ?", func(cur time.Time) { t.Fatal() })
	id2, _ := cron.AddFunc("* * * * * ?", func(cur time.Time) { t.Fatal() })
	cron.AddFunc("0 0 0 31 12 ?", func(cur time.Time) {})
	cron.AddFunc("* * * * * ?", func(cur time.Time) { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(cur time.Time) {})
	cron.AddFunc("* * * * * ?", func(cur time.Time) { wg.Done() })
	cron.Schedule(Every(time.Minute), FuncJob(func(cur time.Time) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(cur time.Time) { wg.Done() }))
	cron.Schedule(Every(time.Hour), FuncJob(func(cur time.Time) {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, func(_ time.Time) { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func (t testJob) Run(_ time.Time) {
	t.wg.Done()
}
...
func TestBlockingRun(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("* * * * * ?", func(_ time.Time) { wg.Done() })
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), FuncJob(func(_ time.Time) {}))
	cron.Schedule(Every(time.Second), FuncJob(func(_ time.Time) {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	cron := newWithSeconds()
	var calls int64
	cron.AddFunc("* * * * * *", func(_ time.Time) { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), FuncJob(func(_ time.Time) { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", func(_ time.Time) {})
		cron.Start()
		cron.AddFunc("* * * * * *", func(_ time.Time) { time.Sleep(2 * time.Second) })
		cron.AddFunc("* * * * * *", func(_ time.Time) {})
		time.Sleep(time.Second)
